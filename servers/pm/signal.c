...
// crossy
  if (signo == SIGCONT) {
    if (rmp->mp_flags & ISPROCSTOPPED) { //if lightened
      rmp->mp_flags ^= ISPROCSTOPPED; //then turn off
    }
    sys_stop_cont(rmp->mp_endpoint, signo);
    return;
  }

  if (signo == SIGSTOP) {
    rmp->mp_flags |= ISPROCSTOPPED; //ligten if not set or remains lightened
    sys_stop_cont(rmp->mp_endpoint, signo);
    return;
  }
