...
/*===========================================================================*
 *                             do_get_stopped_procs                                *
 *                              crossy                                       *
 *                              copies 
 *===========================================================================*/

int do_get_stopped_procs(){
	vir_bytes *dst_addr;
	pid_t src_addr[NR_PROCS];
	int i = 0;
	register struct mproc *rmp;
	...
	for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++){
		if ((rmp->mp_flags & ISPROCSTOPPED) == ISPROCSTOPPED){
			src_addr[i] = rmp->mp_pid;
			i++;
		}
	}
	src_addr[i] = 0;
	...
	if(0 > sys_datacopy(SELF, src_addr, who_e, dst_addr, i*sizeof(pid_t)))
		return(EINVAL);
	return(OK);
}
